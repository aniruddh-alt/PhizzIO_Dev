...
# Class to stream video feed 
class FormGuide:
    def __init__(self, reps=5, total_set=1, threshold_angle=120):
        self.reps = reps
        self.total_sets = total_set
    ...
    def calculate_angle(self, v1, v0, v2):
        v1 = np.array(v1)
        v0 = np.array(v0)
        v2 = np.array(v2)
    ...
    def update_exercise_data(self):
        self.exercise_data["sets"] = self.sets
        self.exercise_data["reps"] = self.reps
        self.exercise_data["elapsed_time"] = self.elapsed_time
        self.exercise_data["mistakes"] = self.mistakes
        self.exercise_data["status"] = "completed" if self.sets == self.total_sets else "incomplete"
    ...
    def stream(self):
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while self.cap.isOpened():
                ...
                if self.sets >= self.total_sets:
                    self.elapsed_time = time.time() - self.start_time
                    self.update_exercise_data()
                    break
...
class HeelSlides(FormGuide):
    def stream(self ,reps=5,total_sets=1,threshold_angle=100):
        status = None
        count = 0
        side = "left"
        ...
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while self.cap.isOpened():
                ...
                try:
                    ...
                    cv2.putText(image,f"Set: {self.sets} / {total_sets}", (50, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                    #Draw pose landmarks
...
class Squats(FormGuide):
    def stream(self, reps=5, total_sets=1, threshold_angle=140):
        status = None
        count = 0
        side = "left"
        over_extension = False
...
class KneeExtensions(FormGuide):
    def stream(self, reps=6, total_sets=1, threshold_angle=120):
            status = None
            count = 0
            side = "left"
            over_extension = False
...
class ArmExtensions(FormGuide):
    ...
    def stream(self, reps=5, total_sets=1, threshold_angle=120):
        status = None
        count = 0
        side = "left"
        start_time = time.time()
        ...
        angle = 0
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while self.cap.isOpened() and self.sets < self.total_sets:
                ret, frame = self.cap.read()
                frame = cv2.flip(frame, 1)
                image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                image.flags.writeable = False
